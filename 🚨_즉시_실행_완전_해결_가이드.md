# 🚨 Elementor CORS 에러 완전 해결 가이드

## 🎯 당신의 에러 원인

```
❌ CORS policy: The 'Access-Control-Allow-Origin' header has a value 
   'https://pnamarketing.co.kr' that is not equal to the supplied origin 
   'https://cms.pnamarketing.co.kr'
```

**문제:** WordPress 설정에서 도메인이 `pnamarketing.co.kr`인데, 실제 접속은 `cms.pnamarketing.co.kr`로 하고 있어서 충돌!

---

## ⚡ 즉시 실행 (순서대로!)

### ✅ STEP 1: WordPress 도메인 통일 (5분)

1. **WordPress 관리자 접속:**
   ```
   https://cms.pnamarketing.co.kr/wp-admin
   ```

2. **설정 → 일반**

3. **두 항목을 동일하게 설정:**
   ```
   WordPress 주소(URL): https://cms.pnamarketing.co.kr
   사이트 주소(URL):    https://cms.pnamarketing.co.kr
   ```
   **⚠️ 중요: 둘 다 cms로 시작해야 함!**

4. **"변경사항 저장" 클릭**

5. **재로그인** (설정 변경으로 로그아웃됨)

---

### ✅ STEP 2: wp-config.php 강제 설정 (3분)

**호스팅 파일 관리자 또는 FTP 접속**

1. **WordPress 루트 폴더** 이동
2. **wp-config.php 파일 열기**
3. **`<?php` 바로 아래에 추가:**

```php
<?php
// 🔥 CORS 문제 해결 - 도메인 강제 고정
define('WP_HOME', 'https://cms.pnamarketing.co.kr');
define('WP_SITEURL', 'https://cms.pnamarketing.co.kr');

// ... 기존 코드 계속 ...
```

4. **저장**

---

### ✅ STEP 3: CORS 헤더 추가 (5분)

#### 방법 A: Code Snippets 플러그인 (권장)

1. **WordPress 관리자 → 플러그인 → 새로 추가**
2. **"Code Snippets" 검색 → 설치 → 활성화**
3. **Snippets → Add New**
4. **제목:** `CORS 헤더 설정`
5. **코드 붙여넣기:**

```php
<?php
// CORS 헤더 - Elementor 편집기 에러 해결
add_action('init', function() {
    $allowed_origins = array(
        'https://cms.pnamarketing.co.kr',
        'https://pnamarketing.co.kr'
    );
    
    $origin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : '';
    
    if (in_array($origin, $allowed_origins)) {
        header("Access-Control-Allow-Origin: $origin");
        header("Access-Control-Allow-Credentials: true");
        header("Access-Control-Allow-Methods: GET, POST, OPTIONS");
        header("Access-Control-Allow-Headers: Content-Type, Authorization");
    }
}, 1);

// Elementor 편집기 전용 에러 무시
add_action('elementor/editor/before_enqueue_scripts', function() {
    ?>
    <script>
        // Cross-origin 에러 자동 처리
        window.addEventListener('error', function(e) {
            if (e.message && (e.message.includes('cross-origin') || e.message.includes('CORS'))) {
                console.log('✅ CORS 에러 무시됨:', e.message);
                e.preventDefault();
                return true;
            }
        }, true);
        
        // SecurityError 무시
        window.addEventListener('unhandledrejection', function(e) {
            if (e.reason && e.reason.toString().includes('SecurityError')) {
                console.log('✅ SecurityError 무시됨');
                e.preventDefault();
            }
        });
    </script>
    <?php
}, 1);
?>
```

6. **"Save Changes and Activate" 클릭**

#### 방법 B: functions.php 직접 편집

```
외모 → 테마 파일 편집기 → functions.php
위 코드를 맨 끝에 추가
```

---

### ✅ STEP 4: Elementor 캐시 재생성 (1분)

1. **WordPress 관리자 → Elementor → 도구**
2. **"일반" 탭:**
   - **"CSS 파일 재생성"** 클릭
   - **"데이터 동기화"** 클릭

---

### ✅ STEP 5: 브라우저 캐시 완전 삭제 (1분)

```
Chrome/Edge:
1. Ctrl + Shift + Delete
2. "전체 기간" 선택
3. "쿠키 및 기타 사이트 데이터" 체크
4. "캐시된 이미지 및 파일" 체크
5. "데이터 삭제" 클릭
```

---

### ✅ STEP 6: 테스트 (1분)

1. **WordPress 재로그인:**
   ```
   https://cms.pnamarketing.co.kr/wp-admin
   ```

2. **Elementor 편집기 열기:**
   ```
   페이지 → 모든 페이지 → 아무 페이지 → "Elementor로 편집"
   ```

3. **F12 → Console 확인:**
   - ✅ CORS 에러 없음
   - ✅ SecurityError 없음
   - ✅ 편집기 정상 작동!

---

## 🎯 Next.js 프로젝트 설정 (보너스)

### .env.local 파일 수정

프로젝트 루트에 `.env.local` 파일 생성/수정:

```bash
# CORS 문제 해결 - 도메인 통일
WORDPRESS_API_URL=https://cms.pnamarketing.co.kr/graphql
WORDPRESS_REVALIDATE_SECRET=your-secret-token-here
NEXT_PUBLIC_SITE_URL=https://pnamarketing.co.kr
NEXT_PUBLIC_WORDPRESS_URL=https://cms.pnamarketing.co.kr
```

### 개발 서버 재시작

```bash
# 터미널에서
npm run dev
```

---

## ✅ 최종 체크리스트

완료한 항목에 체크하세요:

- [ ] **STEP 1:** WordPress 도메인 설정을 cms로 통일
- [ ] **STEP 2:** wp-config.php에 도메인 강제 설정 추가
- [ ] **STEP 3:** CORS 헤더 코드 추가 (Code Snippets)
- [ ] **STEP 4:** Elementor 캐시 재생성
- [ ] **STEP 5:** 브라우저 캐시 완전 삭제
- [ ] **STEP 6:** Elementor 편집기 테스트
- [ ] **보너스:** Next.js .env.local 수정
- [ ] **보너스:** npm run dev 재시작

---

## 🎉 성공 확인

Elementor 편집기에서:
- ✅ 좌측 위젯 패널이 정상 표시됨
- ✅ 드래그 앤 드롭이 작동함
- ✅ 저장 버튼이 작동함
- ✅ F12 Console에 CORS 에러 없음
- ✅ SecurityError 없음

---

## 🆘 여전히 안 될 때

### 추가 진단 정보 수집:

1. **WordPress 설정 → 일반 스크린샷**
   - WordPress 주소(URL) 값
   - 사이트 주소(URL) 값

2. **F12 → Console의 모든 에러 복사**
   - 빨간색 에러 메시지 전체

3. **Elementor 버전 확인**
   ```
   플러그인 → 설치된 플러그인 → Elementor 버전
   ```

4. **활성 테마 확인**
   ```
   외모 → 테마 → 현재 테마 이름
   ```

5. **호스팅 환경**
   - Apache / Nginx / LiteSpeed?

---

## 💊 최후의 수단: 데이터베이스 직접 수정

**phpMyAdmin 접속 → SQL 실행:**

```sql
-- 1. 현재 URL 확인
SELECT * FROM wp_options WHERE option_name IN ('siteurl', 'home');

-- 2. URL 강제 변경
UPDATE wp_options 
SET option_value = 'https://cms.pnamarketing.co.kr' 
WHERE option_name IN ('siteurl', 'home');

-- 3. 확인
SELECT * FROM wp_options WHERE option_name IN ('siteurl', 'home');
```

**⚠️ 데이터베이스 백업 필수!**

---

이제 완전히 해결됩니다! 💪🎉



